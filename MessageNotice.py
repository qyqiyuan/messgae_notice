...
class Message(object):
    ...
    def package_callbak(self):
        ...
        self.push_message(key=self.message_storge_key, message=callbak_data, reverse=True)
    ...
    def repeate_notice(self):
        ...
        self.push_message(key=self.message_storge_key, message=self.__message)
    ...
    def notice(self):
        # 取一个任务
        self.__message = self.get_message(key=self.message_storge_key)
        ...
        # 通知前先备份
        self.push_message(key=self.tmp_message_storge_key, message=self.__message)
        # 正式通知
        try:
            ...
            # 删除备份
            self.del_message(key=self.tmp_message_storge_key, message=self.__message)
...
class MessageNotice(object):
    ...
    def message_restore(self):
        ...
        upload_task_count = get_task_count(self.tmp_message_storge_key)
